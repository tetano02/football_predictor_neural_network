...
for i in range(2013, 2023):
    ...
    actual_x = np.column_stack([data['Home Value'], data['Away Value']])
...
x_test = np.column_stack([data['Home Value'].values, data['Away Value'].values])
...
while True:
    ...
    print(f"Risultato previsto: {np.argmax(pred)}")
...
while True:
    ...
    print(f"Risultato previsto: {np.argmax(prediction)}")
    print(f"Predizione del modello: {prediction}")
    print("")
