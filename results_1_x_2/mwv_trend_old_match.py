...
for i in range(380):
    ...
    print(f"Predizione del modello sul test set: {pred}")
    print(f"Risultato reale: {ut.convert_result_one_hot(test_y)}")
    print(f"Risultato previsto dal modello: {ut.convert_result(np.argmax(pred))}")
    print("")
    if ut.convert_result_one_hot(test_y) != ut.convert_result(np.argmax(pred)):
